...
TEST_CASE("test_structure")
{
    ...
    Eigen::Matrix<double, 3, Eigen::Dynamic > pointsTriangulated = eight::structureFromTwoViews(k, t1.matrix(), image0, image1);
    
    // Test up to scale
    double scale = points.col(0).z() / pointsTriangulated.col(0).z();
    pointsTriangulated.array() *=  scale;
    

    REQUIRE(pointsTriangulated.isApprox(points, 1e-3));
